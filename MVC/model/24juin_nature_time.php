...
require_once $_SERVER["DOCUMENT_ROOT"] . '/24juin/MVC/model/24juin_year_nature_time.php';
...
class NatureTime extends BaseModel
{
    ...
    {
        $anObject = new YearNatureTime();
        $aYearNatureTime = $anObject->getObjectWhereYearAndIdNatureTime($_SESSION['id_year'], $aNatureTime['id_nature_time']);
        
        $line = '';
        if ($aYearNatureTime != null) {
            $line .= "<tr>";
            if ($filter == 0 && $role != 2) {
    ...
    public function getListOfAllDBObjectsWhere($argument, $operation, $value)
    {
		...
		JOIN teacher_nature_time tnt ON tnt.id_nature_time = nt.id_nature_time WHERE " . $argument . " " . $operation . " " . $value . " ORDER by day";
